#!/usr/bin/env python3
"""
Morning Brief v7 — Uses subagent for job scraping via agent-browser
"""
import os, sys, re, json, datetime, time
from urllib.parse import quote_plus

def spawn_job_scraper():
    """Spawn a subagent to scrape jobs using agent-browser."""
    task = (
        "Use agent-browser to search for internal audit jobs in Mumbai from these sites: "
        "1) Google Jobs (https://www.google.com/search?q=internal+audit+Mumbai&hl=en), "
        "2) Naukri (https://www.naukri.com/internal-audit-jobs-in-Mumbai), "
        "3) Indeed (https://www.indeed.com/jobs?q=internal+audit&l=Mumbai). "
        "For each site, extract up to 5 job listings: title, company, and direct apply link. "
        "Write results to /tmp/job_results.json with format: {\"jobs\": [{\"title\": ..., \"company\": ..., \"link\": ..., \"source\": ...}]}. "
        "Avoid duplicates. Do not use web_search; navigate directly."
    )
    try:
        resp = sessions_spawn(
            task=task,
            runtime="acp",
            agentId="agent-browser",  # assumes agent-browser is installed
            mode="run",
            timeoutSeconds=300
        )
        # The subagent runs and writes file; we wait by polling
        return True
    except Exception as e:
        print(f"Failed to spawn job scraper: {e}", file=sys.stderr)
        return False

def wait_for_job_results(timeout=180):
    """Poll for /tmp/job_results.json until timeout."""
    start = time.time()
...
def fetch_market_data():
    """Use Yahoo Finance chart API with proper headers."""
    try:
        import requests
        headers = {
...
def generate_report():
    ...
    # Spawn job scraper subagent and wait
    report.append("Launching job scraper...")
    if spawn_job_scraper():
        jobs = wait_for_job_results(timeout=300)
        if jobs:
            for i, job in enumerate(jobs, 1):
                report.append(f"{i}. {job['title']} @ {job['company']} [{job.get('source','?')}]")
                if job.get('link'):
                    report.append(f"   Apply: {job['link']}")
        else:
            report.append("No jobs fetched (timeout or empty).")
    else:
        report.append("Failed to start job scraper subagent.")
