#!/usr/bin/env python3
"""
Q1 2026 Expense Dashboard — Single scan, then filter by month
"""
...
# Compute days back to Jan 1
today = dt.now().date()
jan1 = dt(2026, 1, 1).date()
days_back = (today - jan1).days + 1  # inclusive

# Run parser once, output to temp CSV
print(f"Scanning last {days_back} days (Jan 1 to present)...")
...
jan = report(jan_rows, "January")
feb = report(feb_rows, "February")
mar = report(mar_rows, "March")
...
# EMI anomaly
if any('EMI' in str(cats) for cats in [jan['cats'], feb['cats'], mar['cats']]):
    emi_mar = mar['cats'].get('EMI', 0)
    if emi_mar > 50000:
        print(f"• EMI category appeared in March only: ₹{emi_mar:,.2f} (new loan/account)")

print("\nData sources: Gmail Axis Bank alerts via expense_parser.py")
print(f"CSV used: {TEMP_CSV}")
